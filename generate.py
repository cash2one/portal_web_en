...
class MakeCode:
    def __init__(self, user, pwd, server_path, server_bak_path, prjName, module=None, url=None):
        ...
        self.media = None
        # self.prjId = prjId
        # self.oPrj = CProject.objects.get(id=self.prjId)
        # self.lstPrjModule = CPrjModule.objects.filter(project_id=self.prjId)
        # self.sPrjPath = os.path.abspath(os.path.join(check.settings.MEDIA_ROOT, self.oPrj.prjName))
        # self.sPrjServerPath = os.path.abspath(os.path.join(check.settings.MEDIA_ROOT, self.oPrj.prjName, 'server'))
    ...
    def compress(self):
        ...
        for dirpath, dirnames, filenames in os.walk(startdir):
            for filename in filenames:
                ...
                arcname = tmp_filename.replace(self.server_bak_path, "")
    ...
    def _makeApi_url(self, dictModule, dictModule_api):
        ...
        lstLine.append(u'')
        lstLine.append(u'from django.conf.urls import patterns, include, url')
        ...
        for name_en in lstModuleName:
            ...
            lstLine.append(u"")
            lstLine.append(u"urlpatterns += patterns('%s.views'," % name_en)
            ...
            for oApi in lstApi:
                ...
                lstLine.append(u"    url(r'^%s$', '%s')," % (url, funName))

            lstLine.append(u')')
            lstLine.append(u'')

        lstLine.append(u"urlpatterns += patterns('api.views',")
        lstLine.append(u"    url(r'^api/$', 'index'),")
        lstLine.append(u"    url(r'^api/docs/$', 'docs'),")
        lstLine.append(u"    url(r'^api/docs/(?P<json>\w+)/$', 'module'),")
        lstLine.append(u")")
        lstLine.append(u"")
    ...
    def _makeApi_views(self, dictModule_api, dictClassName_moduleName, dictAutoApi):
        ...
        for name_en, lstApi in dictModule_api.items():
            ...
            for oApi in lstApi:
                    ...
                    for oApiPara in lstApiPara:
                        ...
                        if oApiPara["required"]:
                            ...
                            if oApiPara["type"] == u"0":  # string类型
                                lstLine.append(u'        %s = request.POST.get("%s", "")  # %s' % (
                                    oApiPara["name"], oApiPara["name"], oApiPara["explana"]))
                            else:
                                lstLine.append(u'        %s = request.POST.get("%s", 0)  # %s' % (
                                    oApiPara["name"], oApiPara["name"], oApiPara["explana"]))
                else:
                    for oApiPara in lstApiPara:
                        if oApiPara in lstPara:
                ...
                if sTmpResult:
                    lstLine.append(u'        dictResp = {"c": ERR_SUCCESS[0], "m": ERR_SUCCESS[1]%s}' % sTmpResult)
                else:
                    lstLine.append(u'        dictResp = {"c": ERR_SUCCESS[0], "m": ERR_SUCCESS[1]}')
                lstLine.append(
                    u'        return HttpResponse(json.dumps(dictResp, ensure_ascii=False), content_type="application/json")')
                lstLine.append(u'')
            ...
            while True:
                ...
                for oTab in lstTab:
                    ...
                    for oField in lstField:
                        sLine = self._makeField(oTab["className"], oField, lstModelsHead, module,
                                                dictClassName_moduleName)
                    ...
                    sHead = u'from models import %s' % oTab["className"]
                    ...
                    lstSigAdmin.append(u'    # 去掉删除')
                    lstSigAdmin.append(u'    # def has_change_permission(self, request, obj=None):')
                    lstSigAdmin.append(u'    # 去掉增加')
                    lstSigAdmin.append(u'    # def has_add_permission(self, request, obj=None):')
                    lstSigAdmin.append(u'    # 过滤编辑页面外键')
                    lstSigAdmin.append(u'    # def formfield_for_foreignkey(self, db_field, request=None, **kwargs):')

                    lstSigAdmin.append(u'    # 删除单条')
                    lstSigAdmin.append(u'    # def delete_model(self, request, obj):')
                    lstSigAdmin.append(u'    # 删除多条')
                    lstSigAdmin.append(u'    # def delete_models(self, request, queryset):')
                    lstSigAdmin.append(u'    # 保存')
                    lstSigAdmin.append(u'    # def save_model(self, request, obj, form, change):')
                    lstSigAdmin.append(u'    # 保存关联表')
                    lstSigAdmin.append(u'    # def save_related(self, request, form, formsets, change):')
                    lstSigAdmin.append(u'    # 列表过滤')
                    lstSigAdmin.append(u'    # def get_search_results(self, request, queryset, search_term):')
                    lstSigAdmin.append(u'    # 编辑页面显示字段')
                    lstSigAdmin.append(u'    # def get_fieldsets(self, request, obj=None):')
                    lstSigAdmin.append(u'    # 编辑页面自定义检查重载函数')
                    lstSigAdmin.append(u'    # def _create_formsets(self, request, obj, change):')
                    lstSigAdmin.append(u'')
    ...
    def _makeField(self, className, field, lstModule, module, dictClassName_moduleName):
        lstPara = []
        if field["isNull"]:
        ...
        if field.has_key("selectForeignKey_className"):
            if field["selectForeignKey_className"] == className:
                sField = u"    %s = models.ForeignKey('self', %sverbose_name=u'%s')" % (
                    field["name"], sPara, field["explana"])
            else:
                sField = u"    %s = models.ForeignKey(%s, %sverbose_name=u'%s')" % (
                    field["name"], field["selectForeignKey_className"], sPara, field["explana"])
    ...
    def make_settings(self, prjName, dictModule):
        ...
        lstLine.append(u")")
        lstLine.append(u"")
        lstLine.append(u"MIDDLEWARE_CLASSES = (")
        lstLine.append(u"    'django.contrib.sessions.middleware.SessionMiddleware',")
        lstLine.append(u"    'django.middleware.locale.LocaleMiddleware',")
        ...
        lstLine.append(u"    'django.middleware.gzip.GZipMiddleware',")
        lstLine.append(u")")
        lstLine.append(u"")
        ...
        lstLine.append(u"LANGUAGE_CODE = 'zh-cn'")
        ...
        lstLine.append(u"}")
        lstLine.append(u"")
        lstLine.append(u"AUTH_USER_MODEL = 'auth.User'")
        lstLine.append(u"")
...
SERVER_PATH = u'C:\wangbin\code\jianyin\server'

# 备份目录，除api，其它都生成在这个目录下
SERVER_BAK_PATH = u'C:\wangbin\code\jianyin\server_bak'

# 登录综合平台的用户名和密码
USER = u'xxxxxxx'
PWD = u'xxxxx'

BASE_URL = "http://zh.whhuiyu.com"
